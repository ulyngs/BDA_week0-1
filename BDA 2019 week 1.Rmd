...
#the entropy function in the entropy package estimates the Shannon entropy in a variable when given the observed counts, so we count how often each type of gear occurs
gear_frequencies <- mtcars %>% 
  count(gear) %>% 
  mutate(equal_counts = 1) # the maximum entropy is what we'd get if each gear was equally likely to occur - to calculate this, let's add a column we set to always be '1'

#let's have a look
gear_frequencies

# hand the counts to the entropy function - set the unit to 'log2' to get the information entropy measured in bits
ent_actual <- entropy(gear_frequencies$n, unit = "log2")

# to get the max entropy, give it the column where we set equal counts
ent_max <- entropy(gear_frequencies$equal_counts, unit = "log2")
  
# normalized entropy is then the proportion of actual entropy to the maximum possible
...
Here is an example which uses wikipedia data that came from this query:
...
Why is the entropy so high? Because we're treating each possible pagelength as a unique value - an exact page length is unlikely to occur multiple times.

Another approach is to bin our data the some way - just like our histogram does - and then calculate how easy it is for us to guess.

Try to play around with the number of bins and see how it affects the entropy!

```{r}
page_length_frequencies_binned <- page_query %>% 
  mutate(bin = cut_interval(page_len, 30)) %>%  #cut into 30 equally spaced bins
  count(bin, .drop = FALSE) %>%  #cut how many are in each (.drop = FALSE means that we keep the values that were zero)
