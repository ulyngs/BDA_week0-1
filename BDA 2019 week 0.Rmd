...
RStudio projects is practic for making sure R (and yourself) knows where to look for files. Basically, when you open an RStudio project file, R's working directory is automatically set to wherever folder on your computer contains that project file. This makes it much easier to share your work with other people, because you won't be trying to read in files using a filepath that is effectively `~/path/that/only/exist/on/my/computer.csv`.
