...
## Overview
This introductory lecture is purely optional and does not contain anything that you will be assessed on. 
It is intended as a primer for the course and so that we can handle any technical issues before we get into the content. 
...
#### RStudio Projects
The materials for each week contain a 'Project' file, that is, a file ending in the extention .Rproj.

RStudio projects are a fantastic way to make sure R (and yourself) knows where to look for files. Basically, when you open an RStudio project file, R's working directory is automatically set to wherever folder on your computer contains that project file. This makes it *much* easier to share your work with other people, because you won't be trying to read in files using a filepath that is effectively `~/path/that/only/exist/on/my/computer.csv`.

Every week, you should open RStudio by opening the R Project file in the materials for that week - then, you should be able to run the R Markdown files without any trouble. 

### R Markdown
R Markdown is a fantastic tool that allow us to mix normal text written in the very simple 'markdown' syntax (see https://en.wikipedia.org/wiki/Markdown) with chunks of code. It's a bit like Jupyter notebooks, which people use for Python, only better...

An R Markdown document is a plain text file that you can open in any text editor. But when you open it in RStudio, you can run the chunks of code and use it as an interactive notebook. You can also export the document to other formats, such as a Microsoft Word or HTML document, using the 'Knit' button. (You could write your homework assignments this way)

To run code one line at a time, click on a line of code and press 'cmd' + 'return'. To run all the code in a chunk, press 'cmd', 'shift' and 'return'.

When you knit your code into, for example, an HTML file, you can control the output. If you want to have all of the input and the output in your knitted document then leave it as it is:
